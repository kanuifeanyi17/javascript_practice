...
function guessingGame(){
    const secretNumber = (Math.random() * 10);
    const secretWholeNum = Math.floor(secretNumber);
    let maxTry = 3;
    let counter = 0;
    const userGuess = parseInt(prompt("GUESS A NUMBER FROM ONE TO 10", ""), 10);

    if(userGuess === secretWholeNum){
        alert ("Your guess is correct!!!")
    }else if(userGuess > secretWholeNum){
        counter++;
        alert ("Your guess is more than the answer. The secret number is " + secretWholeNum)
        if(counter === maxTry){
            alert ("you have used up your free tries")
        }
    }else if(userGuess < secretWholeNum){
        counter++;
        alert ("Your guess is lesser than the answer. The secret number is " + secretWholeNum)
    }
}
guessingGame();